...
my $dbh = DBI->connect("DBI:Pg:dbname=nominatim;port=5433", "tony", "", {'RaiseError' => 1});
...
my $sth = $dbh->prepare("select osm_id, osm_type, class, type, housenumber, 
	get_address_by_language(place_id,  ARRAY['name']) AS label, calculated_country_code AS country, ST_Y(centroid) as latitude, ST_X(centroid) as longitude from placex WHERE place_id >= ? AND place_id < ?");
